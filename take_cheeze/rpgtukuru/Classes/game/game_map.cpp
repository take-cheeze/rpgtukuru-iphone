...
bool GameMap::load(int mapIndex, DataBase& rpgLdb, const char* folder)
{
	mapId_ = mapIndex;
	rpgLdb_ = &rpgLdb;
...
bool GameMap::isEnableMove(int nowX, int nowY, int nextX, int nextY) const
{
	...
	if (nowX > nextX) {
		...
		nextFlag = DataBase::FLAG_MOVE_RIGHT;
	} else if (nowX < nextX) {
		nowFlag = DataBase::FLAG_MOVE_RIGHT;
		nextFlag = DataBase::FLAG_MOVE_LEFT;
	} else if (nowY > nextY) {
		nowFlag = DataBase::FLAG_MOVE_UP;
		nextFlag = DataBase::FLAG_MOVE_DOWN;
	} else if (nowY < nextY) {
		nowFlag = DataBase::FLAG_MOVE_DOWN;
	...
	|| upperChipNext != 0 && (chipSet.blockUpper[upperChipNext] & DataBase::FLAG_CHARACTER_UP) == 0) {
		...
		|| upperChipNext != 0 && (chipSet.blockUpper[upperChipNext] & nextFlag) == 0) {
			...
			if ((chipSet.blockUpper[upperChipNow] & DataBase::FLAG_WALL) != 0) {
	...
	|| (chipSet.blockLower[lowerChipNext] & nextFlag) == 0) {
		...
		if ((chipSet.blockLower[lowerChipNow] & DataBase::FLAG_WALL) != 0) {
