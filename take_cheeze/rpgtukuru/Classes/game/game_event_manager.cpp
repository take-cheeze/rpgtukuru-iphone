...
: kuto::Task(parent)
, gameField_(field)
, executeChildCommands_(true), currentEventPage_(NULL), encountStep_(0), routeSetChara_(NULL)
{
...
{
	const DataBase& rpgLdb = gameField_->getGameSystem().getRpgLdb();
	const CRpgLmu& rpgLmu = gameField_->getMap()->getRpgLmu();
		...
		if (encountStep_ > 0 && encountRatio >= kuto::random(1.f) && mapInfo.m_saEnemyGroup.GetSize() > 0) {
			...
			for (u32 i = 0; i < mapInfo.m_saEnemyGroup.GetSize(); i++) {
				...
				if (mapInfo.m_BattleMapType != 2 ||
				 (terrainId > 0 && (ldb.saEnemyGroup[enemyGroupId].appearTerrain.size() <= terrainId ||
...
void GameEventManager::updateEventAppear()
{
	...
	for (u32 i = 1; i < system.getRpgLdb().saCommonEvent.GetSize(); i++) {
		...
		const DataBase::CommonEvent& commonEvent = system.getRpgLdb().saCommonEvent[i];
		int pageIndex = 0;
...
void GameEventManager::updateEvent()
{
	...
	for (u32 i = 1; i < system.getRpgLdb().saCommonEvent.GetSize(); i++) {
		...
		if (eventPageInfos_[currentEventIndex_].index > 0) {
			const DataBase::CommonEvent& eventPage = system.getRpgLdb().saCommonEvent[i];
			bool isStart = false;
			switch (eventPage.eventList.condition.nStart) {
			case CRpgEventCondition::kStartTypeAuto:
				if (!waitEventInfo_.enable) {
...
void GameEventManager::executeCommands(const CRpgEventList& eventPage, int start)
{
	...
	for (currentCommandIndex_ = 0; currentCommandIndex_ < eventPage.events.size(); currentCommandIndex_++) {
	...
	// execute command
	for (currentCommandIndex_ = start; currentCommandIndex_ < eventPage.events.size(); currentCommandIndex_++) {
		const CRpgEvent& com = eventPage.events[currentCommandIndex_];
...
void GameEventManager::comOperateTextShow(const CRpgEvent& com)
{
	...
	for (int i = currentCommandIndex_ + 1; i < currentEventPage_->events.size(); i++) {
...
void GameEventManager::comWaitSelectStart(const CRpgEvent& com)
{
	if (selectWindow_->closed()) {
		...
		int value = selectIndex < selectWindow_->getMessageSize()? kuto::crc32(selectWindow_->getMessage(selectIndex).str) : 0;
...
{
	executeChildCommands_ = conditionStack_.top().value == kuto::crc32(com.getStringParam());
}
...
{
	while (loopStack_.top().conditionSize < conditionStack_.size()) {
		conditionStack_.pop();
	}
