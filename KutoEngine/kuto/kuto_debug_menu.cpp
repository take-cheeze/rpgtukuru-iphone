...
void DebugMenu::update()
{
	...
	if (virtualPad->repeat(kuto::VirtualPad::KEY_UP)) {
		cursor_--;
		if (cursor_ < 0) {
			cursor_ = (int)SectionManager::instance()->getSectionHandles().size() - 1;
			scroll_ = kuto::max(0, (int)SectionManager::instance()->getSectionHandles().size() - kMaxRowSize);
		} else {
	...
	if (virtualPad->repeat(kuto::VirtualPad::KEY_DOWN)) {
		cursor_++;
		if (cursor_ >= (int)SectionManager::instance()->getSectionHandles().size()) {
			cursor_ = 0;
			scroll_ = 0;
		} else {
	...
	if (virtualPad->repeat(kuto::VirtualPad::KEY_A)) {
		release();
		SectionManager::instance()->beginSection(SectionManager::instance()->getSectionHandles()[cursor_]->getName().c_str());
	}
}
...
void DebugMenu::render(kuto::Graphics2D* g) const
{
	...
	u32 maxSize = kuto::min((u32)kMaxRowSize, (u32)SectionManager::instance()->getSectionHandles().size());
	for (u32 i = 0; i < maxSize; i++) {
		int index = i + scroll_;
		std::string const& str = SectionManager::instance()->getSectionHandles()[index]->getName();
	...
	if (kMaxRowSize < SectionManager::instance()->getSectionHandles().size()) {
		// draw scroll bar
		kuto::Vector2 size(8.f, 210.f * (float)kMaxRowSize / (float)SectionManager::instance()->getSectionHandles().size());
		kuto::Vector2 pos(320.f - size.x, 30.f + scroll_ * (210.f - size.y) / (SectionManager::instance()->getSectionHandles().size() - kMaxRowSize));
