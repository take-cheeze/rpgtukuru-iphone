...
#ifdef _DEBUG
		if(m_pnCounter==NULL)		throw "Memory Secure Error";
#endif
		//参照カウンタセット
		*m_pnCounter = 1;
		//サイズ
...
//	smart_array( unsigned int nSize ){ smart_array(new P[nSize], nSize); }	///< サイズ指定コンストラクタ
...
#else
		P* p = new P[nSize];
		if(p==NULL)		throw "Memory Secure Error";
	...
	*/
	//=============================================================================
#ifndef _DEBUG
	P& operator[]( unsigned int i ) const { kuto_assert(i < m_nSize); return m_aObject[i]; }	///< 配列のふりをする
#else
	//=============================================================================
	/**
		配列のふりをする。
	...
	*/
	...
	const P* GetPtr() const		{ return m_aObject; }				///< 配列の先頭のポインタを取得(const版)
	const P* GetEndPtr() const	{ return m_aObject + m_nSize; }		///< 配列の終端のポインタを取得(const版)
...
//protected:
	...
	*/
	...
	/// 初期化されているかどうか
	bool IsInit() const {return (m_aObject!=NULL); }
