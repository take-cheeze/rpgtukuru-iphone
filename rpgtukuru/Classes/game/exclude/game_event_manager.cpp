...
, timer_(2) // extra for RPG2003
{
	...
	saveMenu_ = addChild( GameSaveMenu::createTask(gameField_) );
	saveMenu_->pauseUpdate(true);
	...
	addCommand<CODE_SELECT_END>();
	addCommand<CODE_SHOP>();
	addCommand<CODE_SHOP_IF_START>();
	addCommand<CODE_SHOP_IF_ELSE>();
	addCommand<CODE_SHOP_IF_END>();
	addCommand<CODE_INN>();
	addCommand<CODE_INN_IF_START>();
	addCommand<CODE_INN_IF_ELSE>();
	addCommand<CODE_INN_IF_END>();
...
{
	if (timer_[0].enable && timer_[0].count) timer_[0].count--;

	updateEventAppear();
	if (waitEventInfo_.enable) { updateWaitEvent(); }
	else {
		updateEvent();
		kuto::VirtualPad* virtualPad = kuto::VirtualPad::instance();
		if (
			virtualPad->press(kuto::VirtualPad::KEY_B) &&
			!gameField_->getPlayerLeader()->isMoving() &&
			gameField_->getGameSystem().canOpenMenu()
...
bool GameEventManager::isEventConditionOk(const rpg2k::structure::InstructionCondition& condition)
{
	...
	if (condition.nFlag & rpg2k::structure::InstructionCondition::kFlagTimer) {
		if (!timer_[0].enable || ((timer_[0].count + 59) / 60) > condition.nTimer)
			appear = false;
	}
	return appear;
}
...
void GameEventManager::updateEvent()
{
	...
	rpg2k::structure::Array2D& mapEvents = system.getLMU()[81];
// mapping events
	for(rpg2k::structure::Array2D::Iterator it = eventStates.begin(); it != eventStates.end(); ++it) {
		...
		rpg2k::structure::Array1D* page = system.currentPage(event[5]);
		if(page) {
			pageID = page->getIndex();
			priority = (*page)[34].get<int>();
		} else {
...
// get page number and map map events
	Array2D& mapEv = system.getLMU()[81];
	for(Array2D::Iterator it = mapEv.begin(); it != mapEv.end(); ++it) {
		if( !it->second.exists() ) continue;

		int eventID = it->first, pageID = pageNum_[eventID];
		if(pageID == rpg2k::INVALID_PAGE_ID) continue;
		Array1D& page = mapEv[eventID][5].getArray2D()[pageID];

		ExecInfo info = {
			EV_MAP, eventID,
			0, false,
			rpg2k::EventStart::Type( page[33].get<int>() ), pageID, &( page[52].getEvent() )
		};

		execInfo_[EV_MAP].insert( std::make_pair(eventID, info) );
	}
// map common events
	Array2D& comEv = system.getLDB()[25];
	for(Array2D::Iterator it = comEv.begin(); it != comEv.end(); ++it) {
		if( !it->second.exists() ) continue;

		int eventID = it->first;
		Array1D& cur = it->second;
		if(
			( cur[11].get<int>() != rpg2k::EventStart::CALLED) &&
			( !cur[12].get<bool>() || ( cur[12].get<bool>() && lsd.getFlag(cur[13]) ) )
		) {
			ExecInfo info = {
				EV_MAP, eventID,
				0, false,
				rpg2k::EventStart::Type( cur[11].get<int>() ), 0, &( cur[22].getEvent() )
			};
