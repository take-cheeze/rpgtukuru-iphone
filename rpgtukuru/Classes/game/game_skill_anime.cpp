...
void GameSkillAnime::render()
{
	...
	for (uint i = 0; i < animeFrame.cells.GetSize(); i++) {
		...
		if (setPlayPosition_) {
			...
			if (anime.scope == CRpgLdb::kAnimeScopeSingle) {
				pos.x = cell.x + playPosition_.x;
				pos.y = cell.y + playPosition_.y;
			} else {
				pos.x = cell.x + 160.f;
				pos.y = cell.y + 80.f;
			}
